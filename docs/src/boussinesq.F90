...
! Further modified and customised by Hossein Kafiabad
...
! -----------------------------------------------------------------
  ...
  ! MISC parameters that are not defined in param.F90
...
! -----------------------------------------------------------------
  ...
  real,  dimension(:,:), allocatable :: utseries,vtseries,zztseries
  real,  dimension(:),   allocatable :: timetseries
  integer, dimension(:), allocatable :: mypoints
  integer :: nomypts
  complex :: termzx,termzy,termzz,termtt,tzx,tzy,tzz,ttt
  complex :: u,v,w,c1,c2,c3,gtau(1200,4)
  real :: dmz,dpz,r1,r2,kx,ky,kz,kh,khn,wk2,k
  real :: ts
      
  integer :: ikx,iky,ikz,ikza,ic,isample
  integer :: iseed,nt=0,nt0,ntdump
  integer :: npe1,inrlcount

  external :: init_params,convol,constr
...
! -----------------------------------------------------------------
! Initialize parameters, fields, L, etc.
! -----------------------------------------------------------------
  ! Open run.list file
  if (mype.eq.0) then
     open (iuRESULT,file='run.list',    form='formatted')
  endif
  ! initialising parameters (kxa, kya and kza) and L(:,:,:)
  call init_params()
  ! defining initial condition (look at module init_condition.F90)
  call init_cond(zxok,zyok,zzok,ttok,uk,vk,wk,geok,gw1k,gw2k,zxor,zyor,zzor,ttor,ur,vr,wr,ts)
...
! -----------------------------------------------------------------
! Preps for diagnostics, physical space slices and time series
! -----------------------------------------------------------------
  
  call prep_diagnostics()
  if (realspace_flag.ne.0) call prep_realslice()
...
! -----------------------------------------------------------------
  ...
  call do_diagnostics(zxnk,zynk,zznk,ttnk,geok,gw1k,gw2k,nzxk,nzyk,nzzk,nttk,rhzx,rhzy,rhzz,rhtt,uk,vk,wk,nt)
  
  if (realspace_flag.ne.0) then
     call dump_realspace(zxnk,zynk,zznk,ttnk,uk,vk,wk,zxnr,zynr,zznr,ttnr,ur,vr,wr)
  endif
...
! -----------------------------------------------------------------
  ...
  if (forcing_flag.ne.0.) then
     call force(nzxk,nzyk,nzzk,nttk,gtau,nt)
  endif
...
! -----------------------------------------------------------------
     ...
     if ( (realspace_flag.ne.0) .and. (mod(nt,nrsevery).eq.0) .and. (iRScount.lt.nrsout) ) then
        call dump_realspace(zxnk,zynk,zznk,ttnk,uk,vk,wk,zxnr,zynr,zznr,ttnr,ur,vr,wr)
     endif
  ...
  if (ncwrite==1) then
     call ncdumpout(zxnk,zynk,zznk,ttnk)
  endif ! ncwrite

  if (realspace_flag.ne.0) call close_ncf_realspace() 
...
! Initialises wavenumbers (kxa, kya and kza) and the mask array L(:,:,:)
  use param  
  implicit none
  integer :: ikx,iky,ikz,ikza,jj
  real ::  kx,ky,kz,wk,kh
