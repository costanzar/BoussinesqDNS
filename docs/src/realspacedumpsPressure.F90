...
module realspacedumps2
  ...
  ! vertical slices
  integer, parameter :: kpZZVtot=1, kpZZVbrt=1
  ! Velocities
  ! horizontal slices
  integer, parameter :: kpUUHtot=1, kpVVHtot=1, kpUUHbrc=1, kpVVHbrc=1
  ! vertical slices
  integer, parameter :: kpUUVtot=1, kpVVVtot=1, kpUUVbrc=1, kpVVVbrc=1
  ! Quadratic Nonlinear terms to derive pressure
  ! horizontal slices
  integer, parameter :: kpQDHtot=1, kpQDHbrc=1
  ! vertical slices
  integer, parameter :: kpQDVtot=1, kpQDVbrc=1  
...
subroutine dump_realspace(zxk,zyk,zzk,ttk,uk,vk,wk,zxr,zyr,zzr,ttr,ur,vr,wr)
  ...
  ttk = qdk
  call fftwkr(plan3_ttnk_ttnr,ttk,ttr)
  if (kpQDHbrc.eq.1) call dump_horslice(ttr,qdhbrcid)
  if (kpQDVbrc.eq.1) call dump_verslice(ttr,qdvbrcid)
